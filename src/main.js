Warning: truncated output (original token count: 104177)
Total output lines: 6988
...
window.AppBranding = {
  ...
  orbitAvatar(size = 76, state = 'idle') {
    ...
    const states = ['idle', 'blink', 'happy', 'listening', 'thinking', 'curious', 'excited', 'proud', 'giggling', 'speaking', 'confused', 'error'];
    const safeState = states.includes(state) ? state : 'idle';
    const customBadge = customLogo
      ? `<img class="orbit-companion-badge" src="${AppUtils.escapeHtml(customLogo)}" alt="">`
      : '';
    return `<div class="orbit-companion" data-orbit-state="${safeState}" style="width:${size}px;height:${size}px;" role="img" aria-label="Orbit AI is ${safeState}">
      <span class="orbit-companion-visual" aria-hidden="true">
        <span class="orbit-companion-body"></span>
        <span class="orbit-companion-face"></span>
        ${customBadge}
      </span>
    </div>`;
  }
    ...
    const sections = [
      ...
      { role: 'counselor', title: 'Counselor', icon: 'badge', description: 'Manage assigned students and counseling records', disabled: false,
        perms: [
          ...
          { label: 'View Analytics', key: 'view_analytics', checked: true },
          { label: 'Send Notifications', key: 'send_notifications', …54177 tokens truncated… 'company_admin', 'school_admin'].includes(profile?.role);
  ...
  async _renderStudentChat(main, school) {
    ...
    const submit = async () => {
      ...
      try {
        ...
        send.disabled = false;
        window.AppVoiceInput.setOrbitState(main, 'error');
      }
  ...
  async open(contentItem, school) {
    ...
    const sendMessage = async () => {
      ...
      try {
        ...
        `);
        window.AppVoiceInput.setOrbitState(askPanel, 'error');
      }
