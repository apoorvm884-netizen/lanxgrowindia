...
// ==============================================================
window.SchoolPortalDashboard = {
  async render(main, data, school, profile) {
    const schoolId = school.id;
    const students = await window.StudentService?.getBySchool(schoolId).catch(() => []) || [];
    ...
    const recentLogs = (data.auditLog || []).filter(l => l.detail && l.detail.includes(school.name)).slice(0, 5);

    main.innerHTML = `<div class="fade-in">
      ${profile.role === 'super_admin' ? `<div style="background:#111827;color:#d1d5db;padding:10px 16px;border-radius:var(--radius-md);margin-bottom:16px;display:flex;align-items:center;gap:12px;font-size:12px;">
        <span class="material-symbols-outlined" style="font-size:16px;">admin_panel_settings</span>
        <span style="flex:1;">SUPER ADMIN MODE — Viewing <strong>${school.name}</strong></span>
        <button class="btn btn-sm" style="background:#374151;color:#fff;border:none;height:28px;font-size:11px;" data-action="navigate" data-route="schools">Exit</button>
      </div>` : ''}
      <div style="background:linear-gradient(135deg,#1e3a8a,#3b82f6);border-radius:var(--radius-xl);padding:28px 32px;color:#fff;margin-bottom:20px;">
        <div style="display:flex;align-items:center;gap:12px;">
          <div style="width:48px;height:48px;border-radius:var(--radius-lg);background:rgba(255,255,255,0.15);display:flex;align-items:center;justify-content:center;font-size:20px;font-weight:700;">${AppUtils.getInitials(school.name)}</div>
          <div><div style="font-size:20px;font-weight:700;">${school.name}</div>
          <div style="font-size:12px;opacity:0.8;margin-top:2px;">Principal: ${school.principal_name || '—'} · Code: ${school.code || '—'} · ${students.length} Students · ${counselors.length} Staff</div></div>
        </div>
      </div>
      <div class="metrics-grid" style="grid-template-columns:repeat(4,1fr);">
        <div class="metric-card" style="cursor:pointer;" data-action="navigate" data-route="school-students">
          <div class="metric-icon metric-icon-green"><span class="material-symbols-outlined">groups</span></div>
          <div class="metric-info"><h2>${students.length}</h2><p>Students</p></div>
        </div>
        <div class="metric-card" style="cursor:pointer;" data-action="navigate" data-route="school-counselors">
          <div class="metric-icon metric-icon-blue"><span class="material-symbols-outlined">badge</span></div>
          <div class="metric-info"><h2>${counselors.length}</h2><p>Counselors</p></div>
        </div>
        <div class="metric-card" style="cursor:pointer;" data-action="navigate" data-route="school-courses">
          <div class="metric-icon metric-icon-purple"><span class="material-symbols-outlined">school</span></div>
          <div class="metric-info"><h2>${data.courses?.length || 0}</h2><p>Courses</p></div>
        </div>
        <div class="metric-card" style="cursor:pointer;" data-action="navigate" data-route="school-content">
          <div class="metric-icon metric-icon-orange"><span class="material-symbols-outlined">videocam</span></div>
          <div class="metric-info"><h2>${content.length}</h2><p>Videos</p></div>
        </div>
      </div>
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:16px;">
        <div class="card">
          ...
          <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;">
            <button class="btn btn-secondary" style="height:44px;font-size:12px;justify-content:flex-start;gap:8px;" data-action="navigate" data-route="school-students"><span class="material-symbols-outlined" style="font-size:18px;color:var(--primary);">person_add</span> Add Student</button>
            <button class="btn btn-secondary" style="height:44px;font-size:12px;justify-content:flex-start;gap:8px;" data-action="navigate" data-route="school-counselors"><span class="material-symbols-outlined" style="font-size:18px;color:var(--primary);">person</span> Manage Counselors</button>
            <button class="btn btn-secondary" style="height:44px;font-size:12px;justify-content:flex-start;gap:8px;" data-action="navigate" data-route="school-courses"><span class="material-symbols-outlined" style="font-size:18px;color:var(--primary);">school</span> Create Course</button>
            <button class="btn btn-secondary" style="height:44px;font-size:12px;justify-content:flex-start;gap:8px;" data-action="navigate" data-route="school-videos"><span class="material-symbols-outlined" style="font-size:18px;color:var(--primary);">video_library</span> Upload Video</button>
            <button class="btn btn-secondary" style="height:44px;font-size:12px;justify-content:flex-start;gap:8px;" data-action="navigate" data-route="school-reports"><span class="material-symbols-outlined" style="font-size:18px;color:var(--primary);">bar_chart</span> View Reports</button>
            <button class="btn btn-secondary" style="height:44px;font-size:12px;justify-content:flex-start;gap:8px;" data-action="navigate" data-route="school-intelligence"><span class="material-symbols-outlined" style="font-size:18px;color:var(--primary);">analytics</span> Intelligence</button>
          </div>
        </div>
        <div class="card">
          ...
          : `<div style="display:flex;flex-direction:column;gap:0;">${recentLogs.map(l => {
            const dotColors = { created: '#10b981', edited: '#3b82f6', uploaded: '#8b5cf6', deleted: '#ef4444', suspended: '#f59e0b' };
            return `<div style="display:flex;align-items:flex-start;gap:10px;padding:10px 0;border-bottom:1px solid var(--border);">
              <div style="width:8px;height:8px;border-radius:50%;background:${dotColors[l.action] || '#94a3b8'};margin-top:6px;flex-shrink:0;"></div>
              <div style="flex:1;min-width:0;">
                <div style="font-size:13px;font-weight:500;">${l.user_name}</div>
      ...
      ${recentStudents.length > 0 ? `<div class="card">
        <div class="card-header"><h3 class="card-title">Recent Students</h3><button class="btn btn-secondary btn-sm" data-action="navigate" data-route="school-students">View All</button></div>
        <div class="table-container"><table><thead><tr><th>Name</th><th>Email</th><th>Status</th><th>Enrolled</th></tr></thead><tbody>
          ${recentStudents.map(s => `<tr><td><div style="display:flex;align-items:center;gap:8px;"><div class="user-avatar" style="width:28px;height:28px;font-size:10px;">${AppUtils.getInitials(s.name)}</div><span class="font-semibold">${s.name}</span></div></td><td style="font-size:13px;">${s.email || '—'}</td><td><span class="status-badge ${s.status === 'active' ? 'status-active' : 'status-suspended'}">${s.status}</span></td><td style="font-size:13px;color:var(--text-secondary);">${AppUtils.formatDate(s.created_at)}</td></tr>`).join('')}
        </tbody></table></div>
      </div>` : ''}
    </div>`;
    initIcons();
